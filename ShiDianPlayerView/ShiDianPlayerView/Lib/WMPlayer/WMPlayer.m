...
//        }];
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(finish) name:AVPlayerItemDidPlayToEndTimeNotification object:self.player.currentItem];
}


-(void)finish{
    
    if(self.FinishBlock != nil) {self.FinishBlock();}
    
    if ([self.delegate respondsToSelector:@selector(player_didFinished)]){
        [self.delegate player_didFinished];
    }
}
...
-(void)play{
    [self.player play];
//    [self PlayOrPause:self.playOrPauseBtn];
    if ([self.delegate respondsToSelector:@selector(player_didPlay)]){
        [self.delegate player_didPlay];
    }
    
    if(self.PlayBlock != nil) {self.PlayBlock();}
}

-(void)pause{
    [self.player pause];
//    [self PlayOrPause:self.playOrPauseBtn];
    if ([self.delegate respondsToSelector:@selector(player_didPause)]){
        [self.delegate player_didPause];
    }
    if(self.PauseBlock != nil) {self.PauseBlock();}
}
...
-(void)reset{
    ...
    if ([self.delegate respondsToSelector:@selector(player_didReset)]){
        [self.delegate player_didReset];
    }
    
    if(self.ResetBlock != nil) {self.ResetBlock();}
